/**
 * Continuation program, like the program for pure shear flows.
 *
...
int main(int argc, char* argv[]) {
    ...
    {
        ArgList args(argc, argv, "parametric continuation of invariant solutions in DDC");
        ...
        DDCFlags ddcflags(args, searchflags.laurette);
        ...
        string uname(""), tname(""), sname(""), restartdir[3];
        if (restart) {
            ...
            uname =
                args.getstr(3, "<flowfield>", "velocity field of initial solution from which to start continuation");
            tname =
                args.getstr(2, "<flowfield>", "temperature field of initial solution from which to start continuation");
            sname =
                args.getstr(1, "<flowfield>", "salinity field of initial solution from which to start continuation");
        }
        args.check();
        ...
        dsi = unique_ptr<ddcDSI>(new ddcDSI());

        if (restart) {
            cout << "Restarting from previous solutions. Please be aware that DNSFlags and DDCFlags "
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                temp[i] = FlowField(restartdir[i] + "tbest", cfmpi);
                salt[i] = FlowField(restartdir[i] + "sbest", cfmpi);
                if (i == 0) {
            ...
            cout << "Loading flags from " + restartdir[0] + "dnsflags.txt and " + restartdir[0] +
                        "ddcflags.txt, neglecting command line switches."
                 << endl;
            ddcflags.load(cfmpi->taskid(), restartdir[0]);
            dt = TimeStep(ddcflags);
            ...
            temp[1] = FlowField(tname, cfmpi);
            salt[1] = FlowField(sname, cfmpi);
            project(ddcflags.symmetries, u[1], "initial value u", cout);
            project(ddcflags.tempsymmetries, temp[1], "initial value temp", cout);
            project(ddcflags.saltsymmetries, salt[1], "initial value salt", cout);
            fixdivnoslip(u[1]);
        ...
        dsi = unique_ptr<ddcDSI>(new ddcDSI(ddcflags, sigma[0], 0, dt, Tsearch, Rxsearch, Rzsearch, Tnormalize,
                                            Unormalize, u[0], temp[0], salt[0], N->getLogstream()));
        ...
        cout << "T     == " << T[0] << endl;
        cout << "dPdx  == " << ddcflags.dPdx << endl;
        cout << "dt    == " << dt.dt() << endl;
        cout << "DDCFlags == " << ddcflags << endl << endl;

        dsi->setPhaseShifts(xphasehack, zphasehack, uUbasehack);
        dsi->chooseMuDDC(muname);
        if (!restart) {
            mu[1] = dsi->mu();
        ...
        for (int i = 0; i <= 2; ++i) {
            dsi->updateMu(mu[i]);
            dsi->makeVectorDDC(u[i], temp[i], salt[i], sigma[i], T[i], x[i]);
        }
