...
// FlowField totalSanility(const FlowField& salt, const DDCFlags flags);
// Real buoyPowerInput(const FlowField& u, const FlowField& temp, const FlowField& salt, const DDCFlags flags, bool relative = true);
// Real dissipation(const FlowField& u, const DDCFlags flags, bool normalize = true, bool relative = true);
// Real heatinflux(const FlowField& temp, const DDCFlags flags, bool normalize = true, bool relative = true);
// Real heatcontent(const FlowField& ttot, const DDCFlags flags);
// Real Nusselt_plane(const FlowField& u, const FlowField& temp, const DDCFlags flags, bool relative = true);
...
//     ddcDSI(DDCFlags& ddcflags, FieldSymmetry sigma, PoincareCondition* h, TimeStep dt, bool Tsearch, bool Ssearch, bool xrelative,
//            bool zrelative, bool Tnormalize, bool Snormalize, Real Unormalize, const FlowField& u, const FlowField& temp, const FlowField& salt,
