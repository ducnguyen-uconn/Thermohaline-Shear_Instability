...
int main(int argc, char* argv[]) {
    cfMPI_Init(&argc, &argv);
    {
        ...
        for (Real t = flags.t0; t <= flags.T; t += dt.dT()) {
            ...
            // fields[0].save("3d_diffusive_convection/u" + i2s(int(t)));//<<--- save only fluctuations
            // fields[1].save("3d_diffusive_convection/t" + i2s(int(t)));
            // fields[2].save("3d_diffusive_convection/s" + i2s(int(t)));

            FlowField u_tot = totalVelocity(fields[0], flags); u_tot.save("3d_diffusive_convection/u" + i2s(int(t)));//<<--- save total fields
            FlowField temp_tot = totalTemperature(fields[1], flags); temp_tot.save("3d_diffusive_convection/t" + i2s(int(t)));
            FlowField salt_tot = totalSalinity(fields[2], flags); salt_tot.save("3d_diffusive_convection/s" + i2s(int(t)));
