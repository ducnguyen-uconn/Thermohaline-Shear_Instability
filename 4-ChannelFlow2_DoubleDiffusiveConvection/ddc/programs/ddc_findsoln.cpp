...
int main(int argc, char* argv[]) {
    ...
    {
        ArgList args(argc, argv, "find an invariant solution in DDC using Newton-Krylov-hookstep algorithm");
        ...
        dsi = unique_ptr<ddcDSI>(new ddcDSI(ddcflags, sigma, 0, dt, Tsearch, Rxsearch, Rzsearch, Tnormalize, unormalize,
                                            u, temp, salt, N->getLogstream()));
        ...
        if (msinit) {
            ...
            for (int i = 1; i < nSh; i++) {
                ...
                string sname_ms = "./Multishooting/" + sname + i2s(i);
                FlowField ui(uname_ms, cfmpi);
                FlowField ti(tname_ms, cfmpi);
                FlowField si(sname_ms, cfmpi);
                u_ms[i] = ui;
                t_ms[i] = ti;
                s_ms[i] = si;
            }
