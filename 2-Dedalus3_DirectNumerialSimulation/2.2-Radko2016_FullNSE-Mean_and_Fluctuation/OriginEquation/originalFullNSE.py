...
To run, restart, and plot using e.g. 4 processes:
    $ mpiexec -n 24 python3 originalFullNSE.py
    $ mpiexec -n 24 python3 originalFullNSE.py --restart
    $ mpiexec -n 24 python3 _.py _/*.h5
"""
import sys
import numpy as np
...
if not restart:
    ...
    sa.fill_random('g', seed=42, distribution='normal', scale=1e-4) # Random noise
...
h_mean = lambda A: d3.Average(A,'x')
timehistory = solver.evaluator.add_file_handler('timehistory', sim_dt=1.0, max_writes=10000, mode=file_handler_mode)
timehistory.add_task(h_mean(u@ex), name='um') # horizontal averaged x-axis velocity
timehistory.add_task(h_mean(u@ez), name='wm') # horizontal averaged z-axis velocity, optional, becasue wm=0
timehistory.add_task(h_mean(te), name='Tm') # horizontal averaged temperature
timehistory.add_task(h_mean(sa), name='Sm') # horizontal averaged sanility
timehistory.add_task(dz(h_mean(sa)), name='S_grad') # horizontally averaged profiles of scalar gradients
timehistory.add_task(dz(h_mean(te)), name='T_grad')
timehistory.add_task(h_mean(u@ez*sa), name='S_conv_flux') # horizontally averaged profiles of scalar convective fluxes
timehistory.add_task(h_mean(u@ez*te), name='T_conv_flux')
timehistory.add_task(h_mean(u@ez*sa)/dz(h_mean(sa)), name='S_tur_diff') # horizontally averaged profiles of turbulent diffusivity
timehistory.add_task(h_mean(u@ez*sa)/dz(h_mean(te)), name='T_tur_diff')
    ...
    while solver.proceed:
        ...
        if (solver.iteration-1) % 5000 == 0:
            ...
            if dist.comm.rank == 0:
                ...
                # plt.show()
                plt.savefig('original/T_time={:010.3f}.png'.format(solver.sim_time), bbox_inches='tight')
                plt.close()
                # plot temperature distribution
                plt.figure(figsize=(10,3))
                ...
                plt.title("t = {:.3f}".format(solver.sim_time))
                # plt.show()
                plt.savefig('original/S_time={:010.3f}.png'.format(solver.sim_time), bbox_inches='tight')
                plt.close()
                ...
                plt.title("t = {:.3f}".format(solver.sim_time))
                # plt.show()
                plt.savefig('original/meanDensity_time={:010.3f}.png'.format(solver.sim_time), bbox_inches='tight')
                plt.close()
            ###########################
