...
To run, restart, and plot using e.g. 4 processes:
    $ mpiexec -n 4 python3 Fig3.py
    $ mpiexec -n 4 python3 Fig3.py --restart
...
tau_u = dist.VectorField(coords, name='tau_u') # velocity
...
# Problem
problem = d3.IVP([p, tau_p, u,tau_u, te, sa], namespace=locals())

# add equations
problem.add_equation("trace(grad_u) + tau_p = 0")
problem.add_equation("integ(p) = 0") # Pressure gauge
problem.add_equation("grad(p) - (Pr/Pe)*lap_u - (4*pi*pi*Ri/(Rp-1))*(te-sa)*ez + tau_u = 0") # should add constant terms
...
checkpoints = solver.evaluator.add_file_handler('checkpoints', sim_dt=500, max_writes=1, mode=file_handler_mode)
